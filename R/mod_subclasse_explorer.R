#' subclasse_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around OntologyApi.search_sub_classes_of (\code{\link[opensilexClientToolsR:OntologyApi]{ search_sub_classes_of }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_subclasse_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' @param api_function_options List of options to pass to OntologyApi.search_sub_classes_of from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:OntologyApi]{ search_sub_classes_of }})
...
mod_subclasse_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      ontologyApi <- OntologyApi$new()
      result <-
        do.call(ontologyApi$search_sub_classes_of, final_options)$data
      result_df <<-
        EnvironmentList_to_dataframe(result)
      itemList <<-
        stats::setNames(result_df$uri, result_df$name)
      
      label <- 'Choose a subclasse:'
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more subclasses:'
      }
