#' variable_detail_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around VariablesApi.search_variables_details (\code{\link[opensilexClientToolsR:VariablesApi]{ search_variables_details }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_variable_detail_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' variable_detail_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to VariablesApi.search_variables_details from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:VariablesApi]{ search_variables_details }})
...
#'  \item{options}{The options used to call VariablesApi.search_variables_details }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of VariablesApi.search_variables_details call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_variable_detail_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(variablesApi$search_variables_details, final_options)$data
      result_df <<-
