#' document_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around DocumentsApi.search_documents (\code{\link[opensilexClientToolsR:DocumentsApi]{ search_documents }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_document_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' document_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to DocumentsApi.search_documents from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:DocumentsApi]{ search_documents }})
...
#'  \item{options}{The options used to call DocumentsApi.search_documents }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of DocumentsApi.search_documents call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_document_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      documentsApi <- DocumentsApi$new()
      result <-
        do.call(documentsApi$search_documents, final_options)$data
      result_df <<-
