#' project_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around ProjectsApi.search_projects (\code{\link[opensilexClientToolsR:ProjectsApi]{ search_projects }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_project_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' project_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to ProjectsApi.search_projects from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:ProjectsApi]{ search_projects }})
...
#'  \item{options}{The options used to call ProjectsApi.search_projects }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of ProjectsApi.search_projects call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_project_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(projectsApi$search_projects, final_options)$data
      result_df <<-
