#' facilitie_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around OrganisationsApi.search_infrastructure_facilities (\code{\link[opensilexClientToolsR:OrganisationsApi]{ search_infrastructure_facilities }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_facilitie_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' facilitie_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to OrganisationsApi.search_infrastructure_facilities from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:OrganisationsApi]{ search_infrastructure_facilities }})
...
#'  \item{options}{The options used to call OrganisationsApi.search_infrastructure_facilities }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of OrganisationsApi.search_infrastructure_facilities call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_facilitie_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      organisationsApi <- OrganisationsApi$new()
      result <-
        do.call(organisationsApi$search_infrastructure_facilities, final_options)$data
      result_df <<-
        EnvironmentList_to_dataframe(result)
      itemList <<-
        stats::setNames(result_df$uri, result_df$name)
      
      label <- 'Choose a facilitie:'
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more facilities:'
      }
