#' germplasm_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around GermplasmApi.search_germplasm (\code{\link[opensilexClientToolsR:GermplasmApi]{ search_germplasm }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_germplasm_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' @param api_function_options List of options to pass to GermplasmApi.search_germplasm from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:GermplasmApi]{ search_germplasm }})
...
#'  \item{options}{The options used to call GermplasmApi.search_germplasm }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of GermplasmApi.search_germplasm call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_germplasm_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      germplasmApi <- GermplasmApi$new()
      result <-
        do.call(germplasmApi$search_germplasm, final_options)$data
      result_df <<-
        EnvironmentList_to_dataframe(result)
      itemList <<-
        stats::setNames(result_df$uri, result_df$name)
      
      label <- 'Choose a germplasm:'
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more germplasms:'
      }
