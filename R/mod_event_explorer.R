#' event_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around EventsApi.search_events (\code{\link[opensilexClientToolsR:EventsApi]{ search_events }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_event_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' @param api_function_options List of options to pass to EventsApi.search_events from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:EventsApi]{ search_events }})
...
#'  \item{options}{The options used to call EventsApi.search_events }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of EventsApi.search_events call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_event_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      eventsApi <- EventsApi$new()
      result <-
        do.call(eventsApi$search_events, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more events:'
      }
