#' device_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around DevicesApi.search_devices (\code{\link[opensilexClientToolsR:DevicesApi]{ search_devices }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_device_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' device_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to DevicesApi.search_devices from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:DevicesApi]{ search_devices }})
...
#'  \item{options}{The options used to call DevicesApi.search_devices }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of DevicesApi.search_devices call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_device_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      # Custom code
      devicesApi <- DevicesApi$new()
      result <-
        do.call(devicesApi$search_devices, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more devices:'
      }
