...
#'
#' @description A shiny::selectInput wrapper around DataApi.search_data_list (\code{\link[opensilexClientToolsR:DataApi]{ search_data_list }})
#'
...
#' @param api_function_options List of options to pass to DataApi.search_data_list from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:DataApi]{ search_data_list }})
...
#'  \item{options}{The options used to call DataApi.search_data_list }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of DataApi.search_data_list call}
#' }
...
mod_data_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      dataApi <- DataApi$new()
      result <-
        do.call(dataApi$search_data_list, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more datas:'
      }
