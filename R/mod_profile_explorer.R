#' profile_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around SecurityApi.search_profiles (\code{\link[opensilexClientToolsR:SecurityApi]{ search_profiles }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_profile_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' profile_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to SecurityApi.search_profiles from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:SecurityApi]{ search_profiles }})
...
#'  \item{options}{The options used to call SecurityApi.search_profiles }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of SecurityApi.search_profiles call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_profile_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(securityApi$search_profiles, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more profiles:'
      }
