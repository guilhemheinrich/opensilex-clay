#' tree_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around OrganisationsApi.search_infrastructures_tree (\code{\link[opensilexClientToolsR:OrganisationsApi]{ search_infrastructures_tree }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_tree_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' tree_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to OrganisationsApi.search_infrastructures_tree from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:OrganisationsApi]{ search_infrastructures_tree }})
...
#'  \item{options}{The options used to call OrganisationsApi.search_infrastructures_tree }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of OrganisationsApi.search_infrastructures_tree call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_tree_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(organisationsApi$search_infrastructures_tree, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more trees:'
      }
