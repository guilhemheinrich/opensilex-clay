#' categorie_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around FactorsApi.search_categories (\code{\link[opensilexClientToolsR:FactorsApi]{ search_categories }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_categorie_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' categorie_explorer Server Functions
#'
#' @export
#' @param id Internal parameter for {shiny}.
#' @param authentification_module Authentification module from this package (\code{\link{mod_authentification_server}})
#' @param api_function_options List of options to pass to FactorsApi.search_categories from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:FactorsApi]{ search_categories }})
...
#'  \item{options}{The options used to call FactorsApi.search_categories }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of FactorsApi.search_categories call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_categorie_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(factorsApi$search_categories, final_options)$data
      result_df <<-
