#' factor_explorer UI Function
#'
#' @description A shiny::selectInput wrapper around FactorsApi.search_factors (\code{\link[opensilexClientToolsR:FactorsApi]{ search_factors }})
#'
#' @param id,input,output,session Internal parameters for {shiny}.
#' @export
#' @param id Environment id of the module
#' @importFrom shiny NS tagList uiOutput
mod_factor_explorer_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' @param api_function_options List of options to pass to FactorsApi.search_factors from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:FactorsApi]{ search_factors }})
...
#'  \item{options}{The options used to call FactorsApi.search_factors }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of FactorsApi.search_factors call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_factor_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(factorsApi$search_factors, final_options)$data
      result_df <<-
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more factors:'
      }
