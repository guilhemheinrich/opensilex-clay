...
#'
#' @description A shiny::selectInput wrapper around ScientificObjectsApi.search_scientific_objects (\code{\link[opensilexClientToolsR:ScientificObjectsApi]{ search_scientific_objects }})
#'
...
#' @param api_function_options List of options to pass to ScientificObjectsApi.search_scientific_objects from opensilexClientToolsR package (\code{\link[opensilexClientToolsR:ScientificObjectsApi]{ search_scientific_objects }})
...
#'  \item{options}{The options used to call ScientificObjectsApi.search_scientific_objects }
#'  \item{selected}{The selection of the shiny::selectInput widget }
#'  \item{choices}{A named list containing the selection choices} 
#'  \item{result_df}{A dataframe containing the results of ScientificObjectsApi.search_scientific_objects call}
#' }
#' @importFrom shiny reactive renderUI
#' @importFrom stats setNames
mod_scientific_object_explorer_server <-   function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$choix <- shiny::renderUI({
      ...
      result <-
        do.call(scientificObjectsApi$search_scientific_objects, final_options)$data
      result_df <<-
        EnvironmentList_to_dataframe(result)
      itemList <<-
        stats::setNames(result_df$uri, result_df$name)
      
      label <- 'Choose a scientific_object:'
      ...
      if (isTruthy(final_widget_options$multiple)) {
        label <- 'Choose one or more scientific_objects:'
      }
